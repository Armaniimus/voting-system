...
class voter{
	...
	public function InsertVote($product_id=0,$vote_id=0){
		...
		if ($_COOKIE['Votid'.intval($product_id)]) return 'You have already voted in past 1h.';
		ob_start();
		$this->db->query('START TRANSACTION');
		$this->db->query('INSERT IGNORE INTO vots (id,val) VALUES('.intval($product_id).','.intval($vote_id).')');
		if ($this->db->affected_rows>0){
			$this->db->commit();
			(!defined('COOKIES_HOST'))?setcookie('Votid'.intval($product_id),'Voted',time()+3600,VOTING_SYSTEM_FOLDER):setcookie('Votid'.intval($product_id),'Voted',time()+3600,VOTING_SYSTEM_FOLDER,COOKIES_HOST);
			ob_end_clean();
